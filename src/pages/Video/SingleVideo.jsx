...
import { useAuth, useHistory, useLike, usePlaylist, useVideo, useWatchLater } from "../../contexts";
import { PlaylistModal, Sidebar, VideoIframe } from "../../components";
import { BsFillEyeFill, BsCheckCircleFill, BsFillHandThumbsUpFill } from "react-icons/bs";
...
export const SingleVideo = () => {
    ...
    const { playlistState: { modalFlag }, playlistDispatch } = usePlaylist();
    ...
    return (
        <div className="container__main container__likes">
            ...
            <Sidebar />
            <div className="video__sec playlist__sec border__rad-4px grid grid__col-70-30">
                <div>
                    ...
                    <div className="video__sec-other">
                        ...
                        <div className="other__sec-title center__flex margin__tb-8px">
                            <h3 className="headline-3 text-heading">{currentVideo?.title}</h3>
                        ...
                        <div className="other__sec-btn center__flex margin__tb-8px">
                            <button className="btns video__cat-btn center__flex margin__tb-8px" onClick={() => authToken ? addToLike(currentVideo) : navigate("/login", { replace: true })} >
                                <p className="margin__lr-4px">Like</p>
                                <BsFillHandThumbsUpFill className="card__btn-like" />
                            </button>
                            <button className="btns video__cat-btn center__flex margin-8px" onClick={() => authToken ? addToWatchLater(currentVideo) : navigate("/login", { replace: true })} >
                                <p className="margin__lr-4px">Watch Later</p>
                                <AiFillClockCircle className="" />
                            </button>
                            <button className="btns video__cat-btn center__flex" onClick={() => authToken ? playlistDispatch({ type: SHOW_MODAL, payload: currentVideo }) : navigate("/login", { replace: true })}>
                                <p className="margin__lr-4px">Save to Playlist</p>
                        ...
                        <div className="other__sec-descr margin__tb-8px">
                            <div className="card__data-lower">
                                <p className="txt-mid">Description : {currentVideo?.description}</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div>
                    <p className="notes__sec">Notes section here...</p>
                </div>
            </div>
        </div>
    )
